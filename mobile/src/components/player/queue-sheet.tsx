  ...
  const items = useMemo(
    () =>
      order
        ...
        .filter(
          (item): item is { orderPosition: number; track: NonNullable<typeof item.track> } =>
            Boolean(item.track),
        ),
    [order, queue],
  );
  ...
  return (
    <View style={[styles.queueRow, current && styles.queueRowCurrent]}>
      ...
      >
        ...
        <Text style={styles.duration}>{formatTime(item.track.durationSec)}</Text>
      </Pressable>
...
const styles = StyleSheet.create({
  ...
  textButtonLabel: { color: colors.accentText, fontSize: font.sm, fontWeight: "700" },
  ...
  empty: { paddingVertical: spacing.xl, color: colors.subtle, fontSize: font.sm },
